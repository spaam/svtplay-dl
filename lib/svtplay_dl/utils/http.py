...
FIREFOX_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36"
...
class HTTP(Session):
    def __init__(self, config={}, *args, **kwargs):
        ...
        self.max_retries = config.get("max_retries") or 5
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise
                logging.debug("Connection error, retrying (%d/%d)", attempt + 1, self.max_retries)
