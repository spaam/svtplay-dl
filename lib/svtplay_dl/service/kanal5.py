...
class Kanal5(Service):
    ...
    def get(self):
        ...
        if self.options.username and self.options.password:
            ...
            self.options.cookies = self.cookies
        ...
        if data["hasSubtitle"]:
            yield subtitle(copy.copy(self.options), "json", "http://www.kanal5play.se/api/subtitles/%s" % video_id)
        ...
        if "streams" in data.keys():
            ...
            if "streams" in data.keys():
                for i in data["streams"]:
                    streams = hlsparse(self.options, self.http.request("get", i["source"]), i["source"])
